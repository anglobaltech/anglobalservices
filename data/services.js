export const servicesMenu = [
  {
    title: "CERTIFICATION & LICENSE",
    items: [
      {
        name: "BIS (ISI MARK) CERTIFICATION FOR DOMESTIC PRODUCTS",
        slug: "isi",
        root: true,
      },
      { name: "BIS ISI MARK FOR FMCS", slug: "FMCS_services", root: true },
      { name: "BIS HALLMARKING CERTIFICATION", slug: "hallmarking", root: true },
      {
        name: "NABL CERTIFICATION SERVICES",
        slug: "nabl_services",
        root: true,
      },
      {
        name: "LABORATORY EQUIPMENT & SETUP",
        slug: "lab_servces",
        root: true,
      },
      { name: "WPC CERTIFICATION SERVICES", slug: "wpc_services", root: true },
    ],
  },
  {
    title: "REGISTRATION",
    items: [
      {
        name: "BIS (CRS) REGISTRATION FOR ELECTRONICS PRODUCTS",
        slug: "crs_electronic_it",
        root: true,
      },
      {
        name: "SOLAR PANEL BIS REGISTRATION SERVICES",
        slug: "solar_services",
        root: true,
      },

      {
        name: "SOLAR PANEL PLANT SETUP & TESTING SERVICES",
        slug: "solar-panel-plant-setup",
        root: true,
      },

      {
        name: "BIS HALLMARKING SETUP & REGISTRATION SERVICES",
        slug: "bis-hallmarking-setup-services",
        root: true,
      },
      {
        name: "BEE REGISTRATION SERVICES",
        slug: "bee_services",
        root: true,
      },
    ],
  },
  {
    title: "",
    items: [
     
      {
        name: "GEM REGISTRATION SERVICES",
        slug: "gem_services",
        root: true,
      },
       {
        name: "TRANING SERVICES (NATIONAL / INTERNATIONAL)",
        slug: "traning_services",
        root: true,
      },
       {
        name: "EPR REGISTRATION SERVICES",
        slug: "epr_services",
        root: true,
      },
      {
        name: "TRADEMARK REGISTRATION SERVICES",
        slug: "trademark_services",
        root: true,
      },
      {
        name: "MSME NSIC REGISTRATION SERVICES",
        slug: "msme_services",
        root: true,
      },
    ],
  },
];
